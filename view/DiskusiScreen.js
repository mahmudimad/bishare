...
class DiskusiScreen extends React.Component {
  constructor() {
    super();
  ...
  await  this.setState({ isFetching: true });
    ...
    var istoko = false;
    if(tuser.tokoid !== undefined && tuser.tokoid !="" )
    istoko = true;
        ...
        .on("value", (snapshot) => {
          ...
          snapshot.forEach((child) => {
            if (
              ...
              if(child.val().diskusitype =="user" || (child.val().diskusitype =="toko" && istoko))
              tuserchats.push({
                key: child.key,
                diskusiid: child.val().diskusiid,
                diskusiname: child.val().diskusiname,
                diskusidesc: child.val().diskusidesc,
                diskusitype: child.val().diskusitype,
              });
            }
          });
      ...
      <TouchableOpacity onPress={async () => { this.OnChatDetail(item) }}>
        ...
        >
          ...
          <View style={{ flex: 2, }}>
            ...
            <Text style={{ marginBottom: 5 }}> {item.diskusidesc} </Text>
      ...
      <View style={styles.container}>
        <SafeAreaView style={{ height: HEIGHT }}>
          ...
          >
            ...
            <Text style={{ fontSize: 16, fontWeight: 'bold', marginTop: 20 }}>Diskusi</Text>
            <View style={{ marginTop: 20 }}>
